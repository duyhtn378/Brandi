$(document).ready(function () {
  // FEEDBACK: Nên có comment đầy đủ từng phần trong JS làm nhiệm vụ gì => thuận tiện cho việc maintain sau này
  ...
  $(document).scroll(function () {
    ...
    if (
      ($(window).scrollTop() > headerEle + featrueEle + workEle * 0.4 - 70 &&
        $(window).scrollTop() < headerEle + featrueEle + workEle) ||
      ($(window).scrollTop() > headerEle + featrueEle + workEle + motEle - 70 &&
        $(window).scrollTop() <
          headerEle + featrueEle + workEle + motEle + factEle)
    ) {
  ...
  // FEEDBACK: Chỗ này có thể làm tốt hơn :). Trong trường hợp dynamic data hoặc có backend can thiệp sẽ ko work
  onBtnBradingClick = () => {
    let identify = "b";
  ...
  // FEEDBACK: Chỗ này có thể làm tốt hơn ( có thể add /remove class )
